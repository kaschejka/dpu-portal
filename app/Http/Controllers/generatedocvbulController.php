...
class generatedocvbulController extends Controller
{
public function generatedoc(Request $req) {
  ...
  $zip->open('storage/temp/'.$req->ndog.'.zip', ZipArchive::CREATE);
      ...
      $date = str_replace($_mD, " ".$_monthsList[$_mD]." ", $date);
          ...
          $dogovorvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\dogovor.docx');
          ...
          if ($req->ulsdg == 'ulendDate') {
            $enddatedog = date("d.m.Y", strtotime($req->ulsrokdog));
          ...
          $dsvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\ds.docx');
          ...
          if ($req->sale_ul) {
            $dspvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\ds_price_a.docx');
          } else {
            $dspvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\ds_price.docx');
          }

          $savedspvip = 'storage/docVB/'.$req->ndog.'/'.$n.'/ДС со стоимостью.docx';
...
if (isset($req->tarif)) {
        ...
        if (array_search('shablon', $req->tarif) > -1) {
          $abcvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\shablon_call\abc.docx');
          ...
          $defvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\shablon_call\def.docx');
        ...
        if (array_search('personal', $req->tarif) > -1) {
          $abcvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\personal_notification\abc.docx');
          ...
          $defvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\personal_notification\def.docx');
          ...
          $defvip->saveas($savedefvip);
          $zip->addFile($savedefvip, 'Персональное уведомление/Приложение DEF '.$req->ndog.'.docx');
                  unset($saveabcvip, $savedefvip, $defvip, $abcvip);
        }

        if (array_search('interact', $req->tarif) > -1) {
          $abcvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\interactive_conversation\abc.docx');
          ...
          $defvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\interactive_conversation\def.docx');
        ...
        if (array_search('kdu', $req->tarif) > -1) {
          ...
          $kduvip = new \PhpOffice\PhpWord\TemplateProcessor(env('DOC_PATH').'generateddoc\shablondogdoc\voicebox\ul\call_8800\kdu.docx');
